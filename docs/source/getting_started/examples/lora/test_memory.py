...
class TestMemoryBudget:
    ...
    def test_rank_mismatch_handling(self):
        ...
        If max_lora_rank=16 but LoRA has rank=32, what happens?
        This tests the error handling for rank mismatch.
        """
        cleanup_gpu()
        ...
        llm = LLM(
            ...
            max_loras=2,
            max_cpu_loras=2,
            max_lora_rank=8,  # Very small rank limit
            gpu_memory_utilization=0.7,
            trust_remote_code=True,
            enforce_eager=True,
        )

        # Try to load a LoRA (actual rank depends on the test LoRA)
        lora_req = LoRARequest("test_lora", 1, get_lora_path(0))

        try:
            result = llm.llm_engine.add_lora(lora_req)
            if result:
                # If it succeeded, the LoRA rank is <= max_lora_rank
                print("INFO: LoRA loaded (rank <= max_lora_rank)")
            else:
                print("INFO: LoRA rejected (rank > max_lora_rank)")
        except Exception as e:
            error_msg = str(e).lower()
            if "rank" in error_msg:
                print(f"INFO: Rank mismatch error (expected): {e}")
            else:
        ...
        print("PASS: Rank mismatch handled correctly")
